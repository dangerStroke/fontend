...
<template>
  <div>
    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;margin-top: 30px;">
      <el-table-column align="center" prop="notice" label="描述" width="300">
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @close="closeDialog">
      <el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
        <el-form-item label="描述" prop="notice">
          <el-input size="small" v-model="editForm.notice" auto-complete="off" placeholder="请输入公告描述"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleEdit: function(index, row) {
      ...
      this.editForm.notice = row.notice
    },
    // 编辑、增加页面保存方法
    submitForm(editData) {
      this.$refs[editData].validate(valid => {
        if (valid) {
          updateNotice(this.editForm)
            .then(res => {
